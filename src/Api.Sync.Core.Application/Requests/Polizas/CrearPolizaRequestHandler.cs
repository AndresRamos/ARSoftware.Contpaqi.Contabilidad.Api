...
public class CrearPolizaRequestHandler : IRequestHandler<CrearPolizaRequest, CrearPolizaResponse>
{
    ...
    public async Task<CrearPolizaResponse> Handle(CrearPolizaRequest request, CancellationToken cancellationToken)
    {
        ...
        if (sdkResult == 0)
        {
            ...
            throw new Exception($"No se pudo crear la poliza. Error: {codigoError} - {mensajeError}");
        }
        ...
        foreach (Movimiento movimiento in poliza.Movimientos.OrderBy(m => m.Numero))
        {
            ...
            _sdkMovimientoPoliza.Diario = !string.IsNullOrWhiteSpace(movimiento.Diario?.Codigo) ? int.Parse(movimiento.Diario.Codigo) : 0;
        ...
        Poliza polizaCreada = await _polizaRepository.BuscarPorIdAsync(_sdkPoliza.Id, request.Options, cancellationToken) ??
                              throw new InvalidOperationException();

        return CrearPolizaResponse.CreateInstance(polizaCreada);
    }
